...
var InstallModules = _helpers.Task.create({
  ...
  return (0, _promise.runSequential)((0, _values["default"])(browser.modules), function (module) {
    ...
    if (files.length < count) {
      return Promise.reject(new Error("Unable to pack ".concat(count - files.length, " module(s)")));
    }

    log.info('Installing package...'); // Install packages

    return (0, _promise.runSequential)(files, function (file) {
      return _npm["default"].install(browser.extension.path, file).then(_npm["default"].createHandler(_vorpal["default"].logger))["catch"](function () {
        return false;
      });
    });
  }).then(function () {
