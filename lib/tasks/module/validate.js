...
function validateDevelopmentDependencies(log, browser, build, module) {
  var valid = true; // Ensure development dependencies match neon-extension-build
...
function validateModules(log, browser, environment, packageNode) {
  ...
  var build = browser.modules['neon-extension-build']; // Validate modules
  ...
  return (0, _promise.runSequential)((0, _values.default)(browser.modules), function (module) {
    ...
    if ((0, _isNil.default)(packageModuleNode)) {
      log.error("[".concat(module.name, "] Unable to find module in package tree"));
      return Promise.reject();
    } // Retrieve module dependency tree


    return (0, _package.getDependencyTree)(module.path).catch(function () {
      return null;
      ...
      log.error("[".concat(module.name, "] ").concat(err));
    });
  }).then(function () {
    if (!valid) {
      return Promise.reject();
