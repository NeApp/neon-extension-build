...
var IgnoredPackages = [/^.*\/fsevents$/, /^(@radon-extension\/build\/)?travis-ci\/underscore.string$/];
...
function validateDependencies(packages, tree) {
  ...
  (0, _forEach.default)(packages, function (pkg, name) {
    ...
    if (name.indexOf('@radon-extension/') === 0 && !(0, _isNil.default)(dependency.integrity)) {
      logError("[".concat(path, "] \"integrity\" field shouldn't be defined"));
      success = false;
    } // Ensure "from" field is valid


    if (pkg.from.indexOf(name) !== 0) {
      logError("[".concat(path, "] \"from\" field isn't valid"));
      success = false;
    } // Ensure dependency version matches


    var version = pkg.from.substring(name.length + 1);

    if (version !== dependency.version) {
      logError("[".concat(path, "] found ").concat(version, ", expected ").concat(dependency.version), ignored);
      success = false;
    } // Update result
