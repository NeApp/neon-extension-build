...
function constructCompiler(browser, environment, buildPath) {
  ...
  _fsExtra.default.writeFileSync(_path.default.join(buildPath, 'webpack.config.js'), _util.default.inspect(configuration, {
    depth: null
...
function writeStats(buildDir, stats) {
  return _fsExtra.default.writeJson(_path.default.join(buildDir, 'webpack.stats.json'), stats.toJson('verbose'));
}
...
var Extension = _helpers.Task.create({
  ...
  var buildPath = _path.default.join(environment.options['build-dir'], browser.name, environment.name); // Construct compiler
  ...
  return runCompiler(compiler).then(function (stats) {
    ...
    writeStats(buildPath, stats); // Exit if there is any errors
