...
var ReleaseFiles = ['contributors.json', 'package.json', 'package-lock.json'];
...
function createReleases(log, browser, version, options) {
  ...
  return (0, _promise2.runSequential)((0, _helpers2.getPackages)(browser), function (module) {
    ...
    return repository.status().then(function (status) {
      ...
      .then(function () {
        var files = (0, _filter.default)(ReleaseFiles, function (name) {
          return _fsExtra.default.existsSync(_path.default.join(module.path, name));
        });
        log.debug("[".concat(module.name, "] Committing changes... (files: ").concat(files.join(', '), ")")); // Commit changes to repository

        return repository.commit("Bumped version to ".concat(version), files).then(function (summary) {
...
function updatePackages(log, browser, version, options) {
  ...
  return (0, _promise2.runSequential)((0, _helpers2.getPackages)(browser), function (module) {
    ...
    function formatVersion(version, name) {
      ...
      return "^".concat(version.substring(0, version.lastIndexOf('.')));
