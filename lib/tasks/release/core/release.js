...
function createRelease(log, module, repository, tag, options) {
  ...
  if (options.dryRun) {
    log.info("Creating release \"".concat(tag, "\" on \"NeApp/").concat(module.name, "\" (skipped, dry run)"));
...
function updatePackageRelease(log, extension, repository, modules, tag, options) {
  ...
  if (options.dryRun) {
    log.info("Updating package release \"".concat(tag, "\" on \"NeApp/").concat(extension.name, "\" (skipped, dry run)"));
    return Promise.resolve();
  } // Retrieve tag details
