...
function getReleaseNotes(module, tag) {
  return _github.GithubApi.repos.getReleaseByTag({
    ...
    return "### [".concat(module.name, "](https://github.com/RadonApp/").concat(module.name, "/releases/tag/").concat(tag, ")\n\n") + "".concat(body);
  }, function () {
    return null;
  });
}
...
function createRelease(log, module, repository, tag, options) {
  ...
  if (options.dryRun) {
    log.info("Creating release \"".concat(tag, "\" on \"RadonApp/").concat(module.name, "\" (skipped, dry run)"));
...
function updatePackageRelease(log, extension, repository, modules, tag, options) {
  ...
  if (options.dryRun) {
    log.info("Updating package release \"".concat(tag, "\" on \"RadonApp/").concat(extension.name, "\" (skipped, dry run)"));
    return Promise.resolve();
  } // Retrieve tag details
