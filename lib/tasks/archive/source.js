...
function updateExtensionManifest(browser, environment) {
  var path = _path.default.join(environment.output.source, 'extension.json'); // Read extension manifest from `path`


  return _json.default.read(path).then(function (pkg) {
    return (// Update extension manifest, and write back to `path`
      _json.default.write(path, _extends({}, pkg, (0, _pick.default)(browser.extension, ['repository', 'travis'])), {
        spaces: 2
      })
    );
  });
...
var SourceArchiveTask = _helpers.Task.create({
  ...
  return (0, _copy.default)(Pattern, browser.path, environment.output.source) // Update extension manifest
  .then(function () {
    return updateExtensionManifest(browser, environment);
