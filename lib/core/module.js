...
function getModulePath(basePath, type, key) {
  ...
  path = _path["default"].resolve(basePath, "".concat(type.directory || '', "radon-extension-").concat(key));
  ...
  path = _path["default"].resolve(basePath, "node_modules/@radon-extension/".concat(key));

  if (_fsExtra["default"].existsSync(path)) {
    return path;
  }

  throw new Error("Unable to find \"".concat(key, "\" module"));
}
...
function overlayModuleManifest(module, browser) {
  return readModuleManifest(module.path, browser).then(function (manifest) {
    return _objectSpread(_objectSpread({}, module.manifest), manifest);
  });
}

function resolve(browser, extension, path, type, id) {
  ...
  Logger.info("Resolving module \"".concat(key, "\" (type: \"").concat(moduleType.name, "\")"));
  var module = {
    id: id,
    key: key,
  ...
  return Promise.resolve(module).then(function (module) {
    return (0, _package.readPackageDetails)(module.path).then(function (pkg) {
      return _objectSpread(_objectSpread(_objectSpread({}, module), pkg), {}, {
        // Package
        "package": pkg
      });
    });
  ...
  .then(function (module) {
    return Promise.resolve().then(function () {
      ...
      var path = _path["default"].join(extension.path, '.modules', "radon-extension-".concat(key));
      ...
      Logger.debug("[".concat((0, _padEnd["default"])(key, 40), "] Repository: ").concat(_util["default"].inspect(repository)));
      ...
      return _objectSpread(_objectSpread(_objectSpread({}, module), (0, _pick["default"])(repository, ['branch', 'commit', 'tag', 'latestTag'])), {}, {
        // Repository
        repository: _objectSpread(_objectSpread({}, module.repository || {}), repository)
      });
    });
  ...
  .then(function (module) {
    ...
    return _objectSpread(_objectSpread(_objectSpread({}, module), (0, _pick["default"])(extension.travis, ['branch', 'commit', 'tag'])), {}, {
      // Travis
  ...
  .then(function (module) {
    return getModuleManifest(extension, module).then(function (manifest) {
      return _objectSpread(_objectSpread(_objectSpread({}, module), manifest), {}, {
        manifest: manifest
      });
    });
  }) // Resolve version
  .then(function (module) {
    return _objectSpread(_objectSpread({}, module), _version["default"].resolve(module));
...
function resolveMany(path, browser, extension) {
  ...
  return (0, _promise.runSequential)((0, _reduce["default"])(extension.modules, function (modules, names, type) {
    ...
    return (0, _keyBy["default"])(modules, 'key');
  });
}
