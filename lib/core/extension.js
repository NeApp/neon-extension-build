...
function resolve(packageDir, packageName) {
  var packagePath = _path.default.join(packageDir, 'Packages', packageName);

  return Promise.resolve({}) // Resolve package details
  .then(function (extension) {
    return (0, _package.readPackageDetails)(packagePath).then(function (pkg) {
  ...
  .then(function (extension) {
    return _git.default.status(packagePath, extension.package.version).catch(function () {
      return {
        ahead: 0,
        dirty: false,
        branch: null,
  ...
  .then(function (extension) {
    return readExtensionManifest(packageName, packagePath).then(function (manifest) {
