...
function writePackageVersions(path, versions) {
  ...
  return _fsExtra.default.readJson(path).then(function (pkg) {
    return (// Write package details
      _fsExtra.default.writeJson(path, updatePackageVersions(pkg, versions))
    );
  });
}
...
function writePackageLockVersions(path, versions) {
  ...
  return _fsExtra.default.readJson(path).then(function (locks) {
    return (// Write package locks
      _fsExtra.default.writeJson(path, updatePackageLockVersions(locks, versions))
    );
  });
}
