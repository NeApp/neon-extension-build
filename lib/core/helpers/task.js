...
function createRunner(task, defaultOptions) {
  return function (_ref4) {
    ...
    return _browser.default.resolve(options['package-dir'], options.browser).catch(function (err) {
      Logger.error("Unable to resolve browser(s): ".concat(err.stack || err.message || err));
      return Promise.reject(err);
    }).then(function (browsers) {
      return (0, _promise.runSequential)(browsers, function (browser) {
        ...
        Logger.info(prefix + getBrowserColour(browser)("Version Name: ".concat(browser.versionName))); // Run task
...
function create(_ref5) {
  ...
  command(_vorpal.default.command(name, description)).option('--build-dir <build-dir>', 'Build Directory [default: ./build]').option('--package-dir <package-dir>', 'Package Directory [default: ./]').option('--browser <browser>', 'Browser [default: all]', Object.keys(_constants.Browsers)).option('--environment <environment>', 'Environment [default: development]', Object.keys(_constants.Environments)).action(createRunner(task, defaultOptions)); // Return task reference
