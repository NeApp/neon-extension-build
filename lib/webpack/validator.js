  ...
  _createClass(Validator, [{
    ...
    value: function processModuleDependency(browser, environment, source, request) {
      ...
      if (dep.name.startsWith('neon-extension-') && !this._isModulePermitted(module, dep.name)) {
        Logger.error("Dependency \"".concat(dep.name, "\" is not permitted for \"").concat(module.name, "\""));
        this._error = true;
        return false;
      } // Find extension dependency


      var extensionDependency = browser.extension.package.dependencies[dep.name]; // Ensure development dependency isn't defined

      if (!(0, _isNil.default)(extensionDependency) && !(0, _isNil.default)(browser.extension.package.devDependencies[dep.name])) {
        Logger.error("Dependency \"".concat(dep.name, "\" shouldn't be defined as a development dependency"));
        this._error = true;
        return false;
      ...
      if (!(0, _isNil.default)(module) && module.type !== 'package') {
        moduleDependency = module.package.dependencies[dep.name]; // Ensure dependency exists
        ...
        if (!(0, _isNil.default)(extensionDependency) && moduleDependency !== extensionDependency) {
          Logger.error("Dependency \"".concat(dep.name, "\" versions should match ") + "(extension: ".concat(extensionDependency, ", ").concat(module.name, ": ").concat(moduleDependency, ")"));
          this._error = true;
          return false;
        }
      ...
      var dependency = moduleDependency || extensionDependency;

      if ((0, _isNil.default)(dependency)) {
        Logger.error("Dependency \"".concat(dep.name, "\" should be defined"));
        this._error = true;
        return false;
      ...
      if (!(0, _isNil.default)(extensionDependency)) {
