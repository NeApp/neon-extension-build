  ...
  _createClass(Validator, [{
    ...
    value: function processModule(browser, environment, module) {
      ...
      module.reasons.forEach(function (source) {
        var sources = _this._getSources(browser, environment, source) || source;

        for (var i = 0; i < sources.length; i++) {
          _this.processModuleDependency(browser, environment, sources[i].module.userRequest, module.userRequest);
        }
      });
    }
  }, {
    ...
    value: function processModuleDependency(browser, environment, source, request) {
      ...
      if (!(0, _isNil.default)(source)) {
        ...
        if ((0, _isNil.default)(module)) {
          Logger.warn("[".concat(dep.name, "] Unknown source: \"").concat(source, "\""));
          return false;
        }
      }
      ...
      if (!dependency.match(DependencyVersionRegex)) {
        if (!(0, _isNil.default)(moduleDependency)) {
          Logger.error("Dependency \"".concat(dep.name, "\" for \"").concat(module.name, "\" \n                    should be pinned to a version (found: ").concat(dependency, ")"));
        } else {
          Logger.error("Dependency \"".concat(dep.name, "\" \n                    should be pinned to a version (found: ").concat(dependency, ")"));
        }
      ...
      if (!(0, _isNil.default)(moduleDependency) && !(0, _isNil.default)(extensionDependency)) {
        Logger.error("Dependency \"".concat(dep.name, "\" has been duplicated \n                (extension: ").concat(extensionDependency, ", ").concat(module.name, ": ").concat(moduleDependency, ")"));
        this._error = true;
        return false;
      } // Mark dependency
      ...
      if (!(0, _isNil.default)(moduleDependency)) {
        (0, _set.default)(this.dependencies, [environment, module.name, dep.name], true);
      } else {
        (0, _set.default)(this.dependencies, [environment, null, dep.name], true);
      ...
      if (!(0, _isNil.default)(module) && module.type !== 'package') {
        ...
        (0, _set.default)(this.peerDependencies, [environment, module.name, dep.name], true); // Ensure peer dependency is defined
        ...
        if (!(0, _isNil.default)(extensionDependency) && !_semver.default.satisfies(extensionDependency, modulePeerDependency)) {
          Logger.error("\"".concat(dep.name, "\" peer dependency in \"").concat(module.name, "\" (").concat(modulePeerDependency, ") \n                    is not satisfied by extension version: ").concat(extensionDependency));
          this._error = true;
          return false;
        }
      }
    ...
    value: function finish(browser, environment) {
      ...
      if ((0, _isNil.default)(this.dependencies[environment]) || (0, _isNil.default)(this.peerDependencies[environment])) {
        return;
      } // Ensure there are no unused extension dependencies


      this._checkDependencies('Dependency', browser.extension.package.dependencies, this.dependencies[environment][null]); // Ensure there are no unused module dependencies


      (0, _forEach.default)((0, _filter.default)(browser.modules, function (module) {
        return module.type !== 'package';
      }), function (module) {
        _this2._checkDependencies('Dependency', module.package.dependencies, _this2.dependencies[environment][module.name], module.name);

        _this2._checkDependencies('Peer dependency', module.package.peerDependencies, _this2.peerDependencies[environment][module.name], module.name);
      });
    }
  }, {
