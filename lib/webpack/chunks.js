...
function getServices(modules, type, options) {
  ...
  (0, _forEach.default)((0, _sortBy.default)(modules, 'name'), function (module) {
    ...
    var servicePath = _path.default.resolve(module.path, "src/services/".concat(name));

    var serviceIndexPath = _path.default.resolve(servicePath, 'index.js'); // Ensure service exists


    if (!_fs.default.existsSync(serviceIndexPath)) {
      Logger.error(_chalk.default.red("Ignoring service \"".concat(name, "\" for module \"").concat(module.name, "\", ") + "no file exists at: \"".concat(serviceIndexPath, "\"")));
      return;
    } // Include service


    items.push(serviceIndexPath); // Include react components (if enabled)

    if (options.includeComponents) {
      var componentsPath = _path.default.resolve(servicePath, 'components/index.js'); // Ensure service components exist
...
function getModuleServices(browser, environment, module) {
  ...
  for (var i = 0; i < module.services.length; i++) {
    ...
    var name = type.substring(type.indexOf('/') + 1); // Build service module path

    var servicePath = _path.default.resolve(module.path, "src/services/".concat(name, "/index.js")); // Ensure service module exists


    if (!_fs.default.existsSync(servicePath)) {
      Logger.error(_chalk.default.red("Ignoring service \"".concat(name, "\" for module \"").concat(module.name, "\", ") + "no file exists at: \"".concat(servicePath, "\"")));
      continue;
