...
function encodeExtensionManifest(browser) {
  return _objectSpread(_objectSpread(_objectSpread({
    ...
    version: null,
    versionName: null
  }, (0, _pick["default"])(browser.extension, ['name', 'version', 'title', 'description', // Required Permissions
  'origins', 'permissions', // Optional Permissions
  'optional_origins', 'optional_permissions'])), (0, _pick["default"])(browser, ['version', 'versionName'])), {}, {
    // Repository Details
    repository: _objectSpread(_objectSpread({}, browser.extension.repository), {}, {
...
function encodeModuleManifests(modules) {
  var manifests = (0, _mapKeys["default"])((0, _mapValues["default"])(modules || {}, function (module) {
    return (0, _pick["default"])(module, ['id', 'key', 'name', 'type', 'title', 'version', 'content_scripts', 'services', // Repository Details
    'repository', // Required Permissions
    'origins', 'permissions', // Optional Permissions
    'optional_origins', 'optional_permissions']);
  }), function (manifest) {
    if (['core', 'package', 'tool'].indexOf(manifest.type) < 0) {
      return "neon-extension-".concat(manifest.type, "-").concat(manifest.id);
    }

    return "neon-extension-".concat(manifest.id);
...
function isSharedDependency(browser, name) {
  return !((0, _isNil["default"])(name) || name.startsWith('@radon-extension/'));
}
...
function createConfiguration(browser, environment) {
  ...
  return {
      ...
      rules: [{
        ...
        use: _extractTextWebpackPlugin["default"].extract({
          ...
          use: [{
            ...
            loader: 'sass-loader',
            options: {
              includePaths: [_fs["default"].realpathSync(_path["default"].resolve(browser.path, 'node_modules/foundation-sites/scss'))]
            }
          }]
        })
      }]
    ...
    new _webpack["default"].optimize.CommonsChunkPlugin({
      ...
      chunks: (_ref = []).concat.apply(_ref, _toConsumableArray((0, _map["default"])(browser.modules, function (module) {
        ...
        if (['source'].indexOf(module.type) >= 0) {
          chunks.push("Plugins/".concat(module.id, "/Main"));
        ...
        (0, _forEach["default"])((0, _pickBy["default"])(module.webpack.modules, function (_ref2) {
          ...
          chunks.push("Plugins/".concat(module.id, "/").concat(name));
        });
        return chunks;
      }))),
      minChunks: function minChunks(module, count) {
      ...
      name: 'Common',
      chunks: ['Background/Common', 'Plugins/Common', 'Application'],
      minChunks: function minChunks(module, count) {
        return shouldExtractModule(browser, environment, module, count, {
          chunk: 'Common'
        });
    ...
    resolve: {
      ...
      'node_modules'],
      alias: _objectSpread(_objectSpread({}, getModuleAliases(browser)), {}, {
